...
namespace GcoopServiceCs
{
    ...
    public class DocumentControl
    {
        ...
        public String NewDocumentNo(String docTypeCode, int year, String connectionString)
        {
            Sta ta = new Sta(connectionString);
            String newDoc = "";
            try
            {
                newDoc = this.NewDocumentNo(docTypeCode, year, ta);
                ta.Close();
            }
            catch (Exception ex)
            {
        ...
        public String NewDocumentNo(String docTypeCode, int year, Sta ta)
        {
            return this.NewDocumentNo(docTypeCode, year, ta, true);
        }

        private String NewDocumentNo(String docTypeCode, int year, Sta ta, bool isNext)
        {
            String resu = "";
            string sql = "select * from cmdocumentcontrol where document_code = '" + docTypeCode + "'";
            Sdt dt = ta.Query(sql);
            if (dt.Next())
            {
                ...
                if (clearType == 1)
                {
                    ...
                    for (int i = 0; i < ni; i++)
                    {
                        ...
                        else if (n[i].ToString().ToUpper() == "R")
                        {
                            ...
                            {
                                lastDoc++;
                                ta.Exe("update cmdocumentcontrol set last_documentno = " + lastDoc + " where document_code = '" + docTypeCode + "'");
                            }
